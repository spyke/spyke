...
TYPE                 // In C:
  ...
  ULNG   = Cardinal; // unsigned long
  LPULNG = ^Cardinal;// ULNG FAR*
  FLT    = single;   // float
...
// in BASIC: Function olDmAllocBuffer& "OLMEM32.DLL" (ByVal usWinFlags%, ByVal ulBufferSize&, hBuffer&)
{               DECLARE_HANDLE(HBUF);
                typedef HBUF FAR* LPHBUF; ( a long pointer to the handle (longint) hbuf
  defn of call: ECODE WINAPI olDmAllocBuffer (UINT, DWORD, LPHBUF);
}
...
Function olDmGetBufferPtr (hbuf : HBUFTYPE; var lpVoid : Pointer): ECODETYPE;
 stdcall; external 'OLMEM32.DLL';
