...
class SpykeFrame(wxglade_gui.SpykeFrame):
    ...
    def plot(self, frametype=None):
        ...
        if frametype == None: # update all visible frames
            ...
            frames = {frametype:self.frames[frametype]}
        ...
        for frametype in frames:
            waves[frametype] = frametype2wave[frametype]
        thread = PlotThread(frames=frames, waves=waves, tref=self.t)
        thread.start()
...
class PlotThread(threading.Thread):
    def __init__(self, frames=None, waves=None, tref=None):
    ...
    def run(self):
        for frametype, frame in self.frames.items():
            if frame.IsShown(): # only update if frame is shown, for performance
                frame.panel.plot(self.waves[frametype], tref=self.tref) # plot it
