"""Simple (bipolar amplitude non-spatiotemporal) event detection loop
implemented in Cython for speed. Kept as a separate module until
Tim Blanche's spatiotemporal event detection algorithms (bipolar amplitude
and dynamic multiphasic) are published
...
cpdef class Detector_Cy:
    """Base Cython Detector class for BipolarAmplitude Cython .searchblock container classes"""
...
cpdef class BipolarAmplitude_Cy(Detector_Cy):
    ...
    cpdef searchblock(self, wave, cutrange, int maxnevents):
        ...
        Uses simple BipolarAmplitude detection without spatiotemporal lockout
        Locks out all the channels on the probe after an event detection
...
cdef find_maxchanii(Settings *s, int maxchanii, int ti, float sign):
    """Update s.maxchanii at timepoint ti over non locked-out chans.
