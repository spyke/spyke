...
cpdef class BipolarAmplitudeFixedThresh_Cy: # maybe this shouldn't inherit from object

    cpdef searchblock_cy(self, wave):
        ...
        cdef ndarray xthresh = np.zeros(nchans, dtype=np.int32) # thresh xing flags
        cdef ndarray last = np.zeros(nchans) # holds last signal value per chan, floats in uV
        cdef ndarray lock = np.zeros(nchans, dtype=np.int32) # holds number of lockout timepoints left per chan

        cdef int tilock = self.tilock # temporal lockout, in num timepoints
        cdef double slock = self.slock # spatial lockout, in um
        cdef ndarray dm = self.dm # Euclidean channel distance matrix
        ...
        #, maxchanii, maxchani, chanj, maxchanchanged
        ...
        for ti from 0 <= ti < nt: # iterate over all timepoints
            for chanii from 0 <= chanii < nchans: # iterate over indices into chans
                if lock[chanii] > 0: # if this chan is still locked out
                    lock[chanii] -= 1 # decr this chan's temporal lockout
                else: # search for a thresh xing or a peak
                    v = absdata[chanii, ti] # TODO: use fast indexing
...
cdef int get_maxchanii(int maxchanii,
                       int nchans,
                       ndarray chans,
                       ndarray dm, # should arrays be passed by ref?
                       double slock,
                       ndarray absdata, # should arrays be passed by ref?
                       int ti):
