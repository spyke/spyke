...
class SpykeWindow(QtGui.QMainWindow):
    ...
    def on_compareAllAutoCorrPairs_clicked(self, RP=750):
        """For each neuron pair, calculate autocorr of each neuron, find number of spike
        intervals <= RP, sum this up for both neurons, then temporarily merge the spike trains
        of the two neurons. Calculate autocorr of merger, and find its number of spike
        intervals <= RP. Take difference between this and the previous sum, normalize by total
        number of ISIs to get change in fraction of refractory period violations df. Sort
        neuron pairs in decreasing order of df. Those with the biggest df have an autocorr
        that's most affected by the incorrect merger. This should help demonstrate that even
        in the worst case scenario, the autocorr is a poor diagnostic of contaminated spike
        trains. Not that in the numerator of the df calcalation, I should really be counting
        number of consecutive ISIs <= RP, not number of spike intervals in general in the
        autocorr <= RP, but the two are very nearly the same thing. If there's one spike in
        that interval, it's very unlikely for there to be another, and so ISI and autocorr for
        such a short interval are effectively the same thing."""
        sort = self.sort
        nids = sort.good
        nn = len(nids)
        ...
        # refractory period violation array, one row per neuron pair:
        # each row: [nid0, nid1, f0, f1, df]
        f = np.zeros((npairs, 5), dtype=np.float64)
        ...
        for nidi0 in range(nn):
            for nidi1 in range(nidi0+1, nn): # for all neuron pairs
                ...
                f0 = len(dts0) / nISI0
                f1 = len(dts1) / nISI1
                df = (len(dts) - (len(dts0) + len(dts1))) / nISI
                f[pairi] = nid0, nid1, f0, f1, df
                pairi += 1
                print('.', end='')
        sortis = np.argsort(f[:, 4])
        ...
        #pl.ylabel(r'$\Delta$f for ISI $\leq$ %.2f ms' % (RP/1000))
        pl.ylabel(r'$\Delta$f')
