...
# otherwise Qt has problems finding the spyke.ui file:
...
getExistingDirectory = QtWidgets.QFileDialog.getExistingDirectory
...
class SpykeWindow(QtWidgets.QMainWindow):
    """spyke's main window, uses gui layout generated by QtDesigner"""
    def __init__(self):
        QtWidgets.QMainWindow.__init__(self)
        self.ui = uic.loadUi('spyke.ui', self)
    ...
    def groupMenuFiltering(self):
        ...
        ui = self.ui
        filteringGroup = QtWidgets.QActionGroup(self)
    ...
    def groupMenuCAR(self):
        ...
        QtDesigner 4.7, so it's done here manually instead"""
        ui = self.ui
        CARGroup = QtWidgets.QActionGroup(self)
    ...
    def groupMenuSampling(self):
        ...
        QtDesigner 4.7, so it's done here manually instead"""
        ui = self.ui
        samplingGroup = QtWidgets.QActionGroup(self)
    ...
    def CreateNewTrack(self):
        ...
        trackfname, _ = getSaveFileName(self, caption=caption,
    ...
    def on_actionOpen_triggered(self):
        ...
        filter = (".dat, .ns6, .srf, .track, .tsf, .mat, .event, .sort & .json files "
                  ...
                  "All files (*.*)")
        fname, _ = getOpenFileName(self, caption="Open stream or sort or din",
                                   directory=self.streampath,
                                   filter=filter)
        fname = str(fname)
        if fname:
    ...
    def on_actionSaveSortAs_triggered(self):
        ...
        fname, _ = getSaveFileName(self, caption="Save sort As",
    ...
    def on_actionExportCSVFile_triggered(self):
        ...
        fname, _ = getSaveFileName(self, caption=caption,
                                   directory=defaultfname,
                                   filter=filter)
        fname = str(fname)
        if fname:
    ...
    def on_actionAboutSpyke_triggered(self):
        ...
        <p>Copyright &copy; 2008-2019 <a href=https://mspacek.github.io>Martin Spacek</a>,
                                                                        Reza Lotun<br>
        ...
        QtWidgets.QMessageBox.about(self, "About spyke", text)
    ...
    def on_clusterButton_clicked(self):
        ...
        if comps and np.all(sids == spikes['id']): # doing PCA/ICA on all spikes
            ...
            for oldcluster in oldclusters:
                oldneuron = s.neurons[oldcluster.id]
                subsidss.append(oldneuron.sids)
    ...
    def randomsplit(self):
        ...
        subsidss = []
        for oldcluster in oldclusters:
            oldneuron = self.sort.neurons[oldcluster.id]
            subsidss.append(oldneuron.sids)
    ...
    def on_ISICleanButton_clicked(self):
        ...
        val = QtWidgets.QMessageBox.question(self, "Remove %d duplicate spikes" % nrm,
    ...
    def GetClusterIDs(self):
        ...
        sw = self.windows['Sort']
        cids = [ i.data() for i in sw.nlist.selectedIndexes() ]
    ...
    def CreateCluster(self, update=True, id=None, inserti=None):
        ...
        s = self.sort
        # if id is None, new autogenerated id is neuron.id:
        neuron = s.create_neuron(id, inserti=inserti)
        sw = self.windows['Sort']
        if update:
            sw.nlist.updateAll()
        cluster = Cluster(neuron.id)
    ...
    def DelClusters(self, clusters, update=True):
        ...
        for cluster in clusters:
            neuron = self.sort.neurons[cluster.id]
            sw.RemoveNeuron(neuron, update=update)
    ...
    def ColorPoints(self, clusters, setnid=None):
        """Color the points that fall within each cluster the same color as the cluster
        itself. Or, if setnid != None, color all points in clusters according to setnid
        value"""
        clusters = toiter(clusters)
...
class MPLWindow(SpykeToolWindow):
    ...
    def __init__(self, parent=None, pos=None, size=None):
        ...
        self.toolbar.message.connect(self.statusBar().showMessage)
        self.resize(*size)
        self.move(*pos)
