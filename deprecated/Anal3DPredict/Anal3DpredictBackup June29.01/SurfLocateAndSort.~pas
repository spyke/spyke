...
*)
  ...
  private
    ...
    Function  Ry(x,y,z : single) : single;
  ...
  public
    ...
    Procedure GetSpikeClusterIDs(var SpikeArray : TSpikeArray);
    Procedure Finish;
  end;
...
begin
  ...
  begin
    ...
    if LocSortForm.ClientWidth < LocSortPanel.Width then
      LocSortForm.ClientWidth := LocSortPanel.Width;
...
{--------------------------------------------------------------------}
Function TLocSortForm.Rx(x,y,z : single) : single;//rotate 3d HORIZ
begin
  Rx := {round}((x-xoff) * cos(theta) + z * sin(theta)) + xoff;
end;

{--------------------------------------------------------------------}
Function TLocSortForm.Ry(x,y,z : single) : single;//rotate 3d VERT
begin
  Ry := {round}((y-yoff) * sin(theta2) - z * cos(theta2)) + yoff;
end;

{--------------------------------------------------------------------}
procedure TLocSortForm.TransForm(x,y,z : single; var tx,ty : integer);//the 3d to 2d transform
begin
  tx := round(Rx(x,y,z));
  ty := round(Ry(x,y,z));
end;
...
begin
  ...
  if good then
  begin
    TransForm(x.mean,y.mean,z.mean,tx,ty);//The 3D to 2D Transform
    if not outofbounds(tx,ty) then
    begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        w := param[c];{SHRT peak for chan c}
        w := w*w;
        sumwx := sumwx + w*x;
        sumwy := sumwy + w*y;
        sumw := sumw + w;
...
begin
  ...
  lnumspikes.caption := IntToStr(index+1);
  ...
  For c := 0 to np-1 do
    SpikeLoc[index].Voltage[c] := Spike.Param[c] * amp * V2uV;
...
{--------------------------------------------------------------------}
Procedure TLocSortForm.GetSpikeClusterIDs(var SpikeArray : TSpikeArray);
var s,maxid : integer;
begin
  maxid := numgoodclusters-1;
  For s := 0 to Length(SpikeLoc)-1 do
  begin
    if (SpikeLoc[s].done and (SpikeLoc[s].ClusterId < numgoodclusters))
      then SpikeArray[s].cluster := SpikeLoc[s].ClusterId
      else SpikeArray[s].cluster := 0;
  end;

  //if you hit stop prematurely then these spikes will not have been clustered
  For s := Length(SpikeLoc) to Length(SpikeArray)-1 do
  begin
    SpikeArray[s].cluster := -1;
  end;
end;

procedure TLocSortForm.FormMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
begin
  if ssRight in Shift then {left + shift buttons down?}
  begin  //translate electrode
   xoff:= X;
   yoff:= Y;
  end else
  if ssLeft in Shift then { left button is down? }
  begin  //rotate electode
    theta := (X/(LocSortForm.Width/2)*90-90)*piby180; //optimise this sucker!
    theta2 := (Y/(LocSortForm.Height/2)*90-90)*piby180; //optimise this sucker!
    Label7.Caption:='X='+inttostr(X)+' theta='+floattostr(theta);
  end;
  if cbrender.checked then Render;
end;
