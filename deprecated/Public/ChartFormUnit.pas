...
begin
  ...
  begin
    ...
    TextOut(PenPos.x, 1, 'FS=' + inttostr(FS) + '猩');
  end;
end;
...
begin
  TotalChans:= 0;
  if CElectrode.Items[CElectrode.ItemIndex] = '然ap54_1a' then //re-map site channel order (of those selected in polytrodeGUI)
    Move(LAYOUT1A, ChanOrder[0], SizeOf(Layout1A)) else
  if CElectrode.Items[CElectrode.ItemIndex] = '然ap54_1b' then //re-map site channel order (of those selected in polytrodeGUI)
    Move(LAYOUT1B, ChanOrder[0], SizeOf(Layout1B)) else
  if CElectrode.Items[CElectrode.ItemIndex] = '然ap54_1c' then //re-map site channel order (of those selected in polytrodeGUI)
    Move(LAYOUT1C, ChanOrder[0], SizeOf(Layout1C)) else
  if CElectrode.Items[CElectrode.ItemIndex] = '然ap54_2a' then //re-map site channel order (of those selected in polytrodeGUI)
    Move(LAYOUT2A, ChanOrder[0], SizeOf(Layout2A)) else
  if CElectrode.Items[CElectrode.ItemIndex] = '然ap54_2b' then //re-map site channel order (of those selected in polytrodeGUI)
